...
const Register: React.FC = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Show immediate validation feedback for email format
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Check if error is about duplicate email or phone
        const errorMessage = result.error || 'Registration failed. Please try again.';
        console.error('Registration failed:', errorMessage);
        
        if (errorMessage.toLowerCase().includes('email already exists') || 
            errorMessage.toLowerCase().includes('email') && errorMessage.toLowerCase().includes('pending')) {
          setErrors({ email: errorMessage });
        } else if (errorMessage.toLowerCase().includes('phone number already exists') || 
                   errorMessage.toLowerCase().includes('phone') && errorMessage.toLowerCase().includes('pending')) {
        ...
        <div className="bg-white dark:bg-gray-800 py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="mt-1 relative">
                <input
                  ...
                  className={`appearance-none block w-full px-3 py-2 pr-10 border rounded-md shadow-sm placeholder-gray-400 dark:placeholder-gray-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm ${
                    ...
                    (formData.email && validateEmail(formData.email)) ? 'border-green-300 dark:border-green-600' :
              ...
              {formData.email && validateEmail(formData.email) && !errors.email && (
                <p className="mt-1 text-sm text-green-600 dark:text-green-400">Email format is valid</p>
              )}
            </div>
